...
//        dump($loglist);die;
    ...
    public function blessings_transfer()
    {
        if ($this->request->isPost()) {
            ...
            if ($number > $this->user['pay_points']) $this->error('粮草不足');
            //用户检测
            ...
            //减少当前用户
            moneyLog($this->user_id,$tranUser['id'],'pay_points',-$number,8,'粮草转增');
            //增加转让用户
            moneyLog($tranUser['id'],$this->user_id,'pay_points',$number,8,'粮草转增');
            //累计充值
